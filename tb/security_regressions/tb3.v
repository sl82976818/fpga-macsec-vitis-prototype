...
module tb;
  ...
  always @(posedge clk) begin
    cyc=cyc+1;
    $display("CYC%0d t=%0t st=%0d in_v=%0d in_r=%0d fbc=%0d plr=%0d out_v=%0d out_d=%h out_last=%0d tag=%h",
      cyc,$time,u_strip.state_reg,sin_valid,in_ready,u_strip.frame_byte_count_reg,u_strip.payload_remaining_reg,
      out_valid,out_data,out_last,tag_o);
  end

  initial begin
    rst=1; repeat(3) @(posedge clk); rst=0; repeat(3) @(posedge clk);
    // CORRUPTED tag: 4 beats total, tlast on beat3
    sin_valid=1; sin_keep=8'hFF; sin_data=64'h1122334455667788; sin_tlast=0; @(posedge clk); sin_valid=0;
    sin_valid=1; sin_keep=8'hFF; sin_data=64'hFFEEDDCCBBAA9988; sin_tlast=0; @(posedge clk); sin_valid=0;
    sin_valid=1; sin_keep=8'hFF; sin_data=64'h7766554433221100; sin_tlast=0; @(posedge clk); sin_valid=0;
    sin_valid=1; sin_keep=8'hFF; sin_data=64'h8877665544332211; sin_tlast=1; @(posedge clk); sin_valid=0; sin_tlast=0;
