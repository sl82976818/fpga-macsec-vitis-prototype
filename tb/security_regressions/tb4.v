...
module tb;
  ...
  always @(posedge clk) begin
    cyc=cyc+1;
      ...
      $display("CYC%0d t=%0t st=%0d plr=%0d out_v=%0d out_d=%h out_last=%0d tag=%h pn=%0d",
        cyc,$time,u_strip.state_reg,u_strip.payload_remaining_reg,out_valid,out_data,out_last,tag_o,pn_o);
    
  

  initial begin
    rst=1; repeat(3) @(posedge clk); rst=0; repeat(3) @(posedge clk);
    $display("=== CORRUPTED tag frame: 40B = 16 payload + 24 tag; last 16B are tag = 0xDEAD...deed ===");
    // beat0-1: 16B payload plaintext 0x1122...
    ...
    // beat2-4: 24B tag tail, last 16B = CORRUPTED tag 0xDEAD..DEAD
    sin_valid=1; sin_keep=8'hFF; sin_data=64'h6172636879747073; sin_tlast=0; @(posedge clk); // 8B PN-ish/tag prefix
    sin_valid=1; sin_keep=8'hFF; sin_data=64'hDEADBEEFDEADBEEF; sin_tlast=0; @(posedge clk); // tag[15:8]
    sin_valid=1; sin_keep=8'hFF; sin_data=64'hDEADBEEFDEADBEEF; sin_tlast=1; @(posedge clk); // tag[7:0] last
    sin_valid=0; sin_tlast=0;
    repeat(40) @(posedge clk);
    $display("=== DONE: see CYC log: out_v=1 beats are RELEASED plaintext despite CORRUPTED tag ===");
    $finish;
